    ...
    (function pretreatment () {//预处理
        ...
        httpGet(httpUrl,param,
            function success(result){
                console.log(result);
                const {id, partyA, partyAIdCard, partyB, partyBIdCard, amount, rate, startRateDay, endRateDay, repayStatus, pactDay} = result.data.loan;
                $('.number').text(id);
                $('.partyA').text(partyA);
                $('.partyAIdCard').text(partyAIdCard);
                $('.partyBIdCard').text(partyBIdCard);
                $('.amount').text(amount);
                let amountD = amount ? moneyToCapital(amount) : '零元';
                $('.amountD').text(amountD);
                $('.partyB').text(partyB);
                $('.rate').text(rate ? Number(rate)/100 : 0);
                $('.repayStatus').text(repayStatus);
                let startRateDayText = startRateDay ? startRateDay.date['year'] + '年' + startRateDay.date['month'] + '月' + startRateDay.date['day'] + '日' : '';
                $('.startRateDay').text(startRateDayText);
                let endRateDayText = endRateDay ? endRateDay.date['year'] + '年' + endRateDay.date['month'] + '月' + endRateDay.date['day'] + '日' : '';
                $('.endRateDay').text(endRateDayText);
